...
mod metrics {
    ...
    fn set_metrics_for(position_label: &str, status: &str, position: &[&Cfd], symbol: &str) {
        ...
        let margin_counterparty = position.iter().fold(Amount::ZERO, |sum_margin, cfd| {
            sum_margin + cfd.margin_counterparty
        });
        ...
        let margin = position
            .iter()
            .fold(Amount::ZERO, |sum_margin, cfd| sum_margin + cfd.margin);
